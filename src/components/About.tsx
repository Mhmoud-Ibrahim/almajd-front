...
// import img from '../assets/خدمة حجز موعد لتجديد الجواز.jpg'
// import { 
...
// استيراد Swiper والموديولات المطلوبة
...
import img from '../assets/خدمة حجز موعد لتجديد الجواز.jpg';
import img2 from '../assets/خلفيات شاشة سطح المكتب موسمي واجهة مستخدم مُبسط جبل ثلجي رمادي وأبيض.jpg';
import img3 from '../assets/White Green Modern Business Conference And Seminar Intro Video.mp4';
import img4 from '../assets/شهادة شكر وتقدير لون ذهبي مع بيج مزخرفة.jpg';
import img5 from '../assets/background-removed-with-bg-background-removed.png_edited.png';
...
const About: React.FC = () => {
  const sliderItems = [img, img2, img3, img4, img5];
        ...
        <div className="row align-items-center mb-5 pb-lg-5">
          ...
          >
            <div className="position-relative p-2">
              {/* خلفية جمالية ثابتة خلف السلايدر */}
              <div className="bg-primary rounded-4 position-absolute w-100 h-100" style={{ top: '15px', right: '-15px', zIndex: 0, opacity: 0.1 }}></div>
              
              {/* سلايدر الصور بتأثير Coverflow */}
              <Swiper
                ...
                autoplay={{
                  delay: 2500, // سرعة التقليب (2.5 ثانية)
                  disableOnInteraction: false, // لا يتوقف عند لمس المستخدم
                }}
                loop={true}
                pagination={{ clickable: true }}
                className="rounded-4 shadow-lg overflow-hidden"
                style={{ zIndex: 1, paddingBottom: '30px' }}
              >
                {sliderItems.map((item, index) => (
                  <SwiperSlide key={index} style={{ width: '100%', backgroundPosition: 'center', backgroundSize: 'cover' }}>
                    {item.endsWith('.mp4') ? (
                      <video 
                        src={item} 
                        autoPlay 
